...
function mapRowToProjectInvitation(row) {
    return {
        id: row.id,
...
export class SqliteProjectInvitationRepository {
    constructor(db) {
        this.db = db;
        this.insertStmt = this.db.prepare(`INSERT INTO project_invitations (project_id, token, created_by_user_id, created_at, expires_at)
       VALUES (@project_id, @token, @created_by_user_id, @created_at, @expires_at)`);
        this.findByTokenStmt = this.db.prepare('SELECT * FROM project_invitations WHERE token = ?');
        this.markAsUsedStmt = this.db.prepare(`UPDATE project_invitations 
    ...
    async create(input) {
        ...
        do {
            token = generateToken();
            const existing = this.findByTokenStmt.get(token);
            if (!existing)
                break;
            attempts++;
            if (attempts > 10) {
        ...
        const created = this.db.prepare('SELECT * FROM project_invitations WHERE id = ?').get(info.lastInsertRowid);
        if (!created) {
    ...
    async findByToken(token) {
        const row = this.findByTokenStmt.get(token);
        if (!row)
            return null;
        // Check if expired
        if (row.expires_at) {
