...
import { ProjectMember, ProjectInvitation, CreateInvitationInput, AcceptInvitationInput, ProjectRole } from './invitation-types';

export interface ProjectRepository {
  create(input: CreateProjectInput): Promise<Project>;
  findById(id: number): Promise<Project | null>;
  listByOwner(userId: number): Promise<Project[]>;
  listByUser(userId: number): Promise<Project[]>; // List all projects where user is a member
