...
function formatSubtotalDiff(diff) {
    return diff >= 0 ? `${diff} remaining` : `Over by ${Math.abs(diff)}`;
}
function sortExportEntries(entries) {
    return [...entries].sort((a, b) => a.entryDate.localeCompare(b.entryDate));
}
export function buildSpendingExcelRows(entries, lots, options) {
    const formatDate = options?.formatDate ?? ((iso) => iso);
    const rows = [[...SPENDING_EXCEL_HEADERS]];
...
export function isSpendingExcelMetaRow(lotName, description, bank, dateRaw, amountRaw) {
    const lot = lotName.trim().toLowerCase();
    const desc = description.trim().toLowerCase();
    const dateEmpty = dateRaw === '' || dateRaw == null;
    const amountEmpty = amountRaw === '' || amountRaw == null;
    if (lot === 'uncategorized' && dateEmpty && amountEmpty)
        return true;
    ...
    if (/^total remaining/i.test(desc))
        return true;
    if (dateEmpty && amountEmpty && !bank.trim())
        return true;
    return false;
}
