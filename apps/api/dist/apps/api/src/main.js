...
app.post('/api/epics/:epicId/tasks', asyncHandler(async (req, res) => {
    ...
    const response = toTaskResponse(created);
...
app.get('/api/projects/:projectId/done-tasks', asyncHandler(async (req, res) => {
    const token = req.cookies[config.security.sessionCookieName];
    if (!token) {
        res.status(401).json({ error: 'auth/no-token', message: 'No token' });
        return;
...
app.patch('/api/projects/:projectId/summary/visibility', asyncHandler(async (req, res) => {
    const token = req.cookies[config.security.sessionCookieName];
    if (!token) {
        res.status(401).json({ error: 'auth/no-token', message: 'No token' });
        return;
...
app.post('/api/projects/:projectId/summary', asyncHandler(async (req, res) => {
    const token = req.cookies[config.security.sessionCookieName];
    if (!token) {
        res.status(401).json({ error: 'auth/no-token', message: 'No token' });
        return;
...
app.delete('/api/summary/:entryId', asyncHandler(async (req, res) => {
    const token = req.cookies[config.security.sessionCookieName];
    if (!token) {
        res.status(401).json({ error: 'auth/no-token', message: 'No token' });
        return;
