...
import { z } from 'zod';
import {
  buildDevisExcelBuffer,
  buildSpendingExcelBuffer,
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const spendingEntries = [...spendingData.entries].sort(
      (a, b) => a.position - b.position || a.id - b.id
    );
    const summaryEntries = [...summaryData.entries].sort(
      (a, b) => a.position - b.position || a.id - b.id
    );
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="${asciiFilename}"; filename*=UTF-8''${encodeURIComponent(zipFilename)}`
    );
